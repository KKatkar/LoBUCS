...
class Worker:
    ...
    def download_files_from_s3(self,s3obj):
        ...
        for key in s3obj.list():
			logging.debug("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
			logging.debug(s3obj.list())
			logging.debug(key)
			logging.debug(key.name)
			key.get_contents_to_filename(directory+'/'+key.name)
			#key.get_contents_to_filename(directory)
    ...
    def process_downloaded_files(self):
        directory = './'+self.customer_id+'/'
        cmd = '/home/ec2-user/ffmpeg-git-20170301-64bit-static/ffmpeg -r 1 -i '+directory+'/b%03d.jpg  -r 25 '+directory+'/out.mpeg'
        ret = os.system(cmd)
        return ret
